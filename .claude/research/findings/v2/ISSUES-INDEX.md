...
|---------|------------|---------------|
| 1 | Maintainability | [ARCHITECTURE-IMPROVEMENT-FINDINGS.md](./ARCHITECTURE-IMPROVEMENT-FINDINGS.md) §1 |
| 2 | Type Safety | [TYPE-SAFETY-DETAILED.md](./TYPE-SAFETY-DETAILED.md) |
| 3 | Prompting Techniques | [ARCHITECTURE-IMPROVEMENT-FINDINGS.md](./ARCHITECTURE-IMPROVEMENT-FINDINGS.md) §3 |
...
| 9 | Skill Architecture | [ARCHITECTURE-IMPROVEMENT-FINDINGS.md](./ARCHITECTURE-IMPROVEMENT-FINDINGS.md) §9 |
...
| 12 | Testing & Verification | [ARCHITECTURE-IMPROVEMENT-FINDINGS.md](./ARCHITECTURE-IMPROVEMENT-FINDINGS.md) §12 |
...
**Status:** ✅ **RESOLVED** - 17% context usage in worst case is acceptable for short-lived subagents
...
**Status:** ✅ **RESOLVED** - All skills now have proper YAML frontmatter

**Original Claim:** The `skill: "frontend-api"` syntax doesn't work. There's no Skill tool.

**Resolution:**
All skill source files in `src/skills/` have been updated with proper YAML frontmatter in the format:
```yaml
---
name: skill-name
...
- **Frontend (8):** react, styling, api, client-state, accessibility, performance, testing, mocking
- **Backend (10):** api, authentication, database, ci-cd, performance, testing, analytics, feature-flags, email, observability
- **Setup (7):** monorepo, package, env, tooling, posthog, resend, observability
- **Shared/Security (2):** reviewing, security
- **Research (1):** methodology

**What Was Done:**
1. Audited all skill files in the central skills repository
2. Added YAML frontmatter with `name` and `description` fields to each
3. Used the name/description from metadata.yaml for consistency
...
**Source:** Agent #1 (Maintainability)
**Status:** 📋 **PLANNED** - Scaffolding CLI will be implemented (see ISSUE-7-8-PROPOSALS.md)
...
- Updates `agents.yaml`
- Updates all selected `stacks/{stack}/config.yaml`
...
**Source:** Agent #9 (Skill Architecture)
**Status:** ❌ **NOT NEEDED** - Git history sufficient, no external consumers
...
**Source:** Agent #9 (Skill Architecture)
**Status:** ❌ **NOT NEEDED** - Skills are self-contained, bundles solve grouping
...
**Source:** Agent #3 (Prompting Techniques)
Binary precompiled/dynamic. Should have metadata→instructions→examples tiers.

---

#### 37. Negative Framing in anti-over-engineering.md
**Source:** Agent #3 (Prompting Techniques)
Uses "❌ Don't create" patterns. Should be "✅ Use existing".
...
>
> **Note (2026-01-08):** Issue #4 was RESOLVED by adding proper YAML frontmatter to all 36 skill source files in the central skills repository. Skills now have the required `name` and `description` fields for Claude Code's native Skill tool discovery.
