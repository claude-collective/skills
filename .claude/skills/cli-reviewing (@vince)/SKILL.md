...
```typescript
// ✅ Good Example - Named constant with clear meaning
process.exit(EXIT_CODES.VALIDATION_ERROR);
```

**Why good:** Exit code meaning is documented, scripts can reliably check for specific error types, maintainable as team grows

```typescript
// ❌ Bad Example - Magic number exit code
process.exit(1); // What does 1 mean?
```

**Why bad:** Undocumented exit codes are unmaintainable, scripts depending on CLI can't reliably detect error types, meaning lost when developer leaves
...
```typescript
// ✅ Good Example - Proper cancellation handling
...
**Why good:** Graceful exit on Ctrl+C, user sees clear cancellation message, correct exit code for scripting integration

```typescript
// ❌ Bad Example - Missing isCancel check
const name = await p.text({ message: "Name:" });
// User presses Ctrl+C - name is Symbol, code continues with garbage
```

**Why bad:** User cancellation causes Symbol value to propagate, subsequent code crashes or behaves unexpectedly, no graceful exit path
...
```typescript
// ✅ Good Example - Spinner with descriptive messages
const s = p.spinner();
...
**Why good:** User sees progress during async operation, success message provides useful info, spinner properly stopped before error logging, named exit code constant

```typescript
// ❌ Bad Example - No feedback for long operation
const data = await fetchRemoteConfig(); // User sees nothing
```

**Why bad:** User sees no progress, appears frozen/broken, no way to know if operation is working or stuck
...
```typescript
// ✅ Good Example - Actionable error with resolution
p.log.error(`Config file not found: ${configPath}`);
p.log.info(`Run 'mycli init' to create one, or specify path with --config`);
```

**Why good:** User knows WHAT failed, WHERE (path), and HOW to fix it (two options provided)

```typescript
// ❌ Bad Example - Unhelpful error
p.log.error("Failed");
```

**Why bad:** No information about what failed, user has no path to resolution, creates support burden

```typescript
// ⚠️ Acceptable but could be better
p.log.error(`Config file not found: ${configPath}`);
// Missing: how to fix it
```

**Why acceptable:** Says what and where, but missing resolution guidance
...
**CLI Reviewing integrates with other skills for comprehensive code reviews.**

**Works with:**

- **cli-commander (@vince)**: Reference implementation patterns when suggesting fixes
- **reviewing (@vince)**: General feedback principles and severity classification
- **testing (@vince)**: CLI-specific testing adequacy criteria

**Depends on:**

- **cli-commander (@vince)**: Required for understanding correct CLI patterns (exit codes, signal handling, prompts)

**Review Handoff:**

- For general code quality issues (naming, structure), defer to **reviewing (@vince)**
- For security vulnerabilities beyond CLI-specific (auth, injection), defer to **security (@vince)**
- For performance issues, defer to appropriate performance skills
...
- ⚠️ No spinner for operations likely to exceed 500ms
- ⚠️ Error message says what failed but not how to fix it
- ⚠️ Missing `--dry-run` support for destructive operations
- ⚠️ No verbose mode for debugging
- ⚠️ Config precedence incorrect (env overrides flag)
- ⚠️ Missing validation for user input in prompts
- ⚠️ No `showHelpAfterError(true)` configured
