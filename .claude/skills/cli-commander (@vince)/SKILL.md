...
- Simple scripts without user interaction (use plain Node.js with process.argv)
- One-off automation scripts (may not need Commander structure)
- Browser-based interfaces (use web frameworks instead)
- APIs or servers (use Hono or Express)
...
- **setup/tooling (@vince)**: ESLint, TypeScript, Prettier configuration for CLI projects
- **vitest (@vince)**: Testing Commander commands with mocked fs and prompts
- **axiom+pino+sentry (@vince)**: Structured logging for CLI output, error tracking

**Replaces / Conflicts with:**

- **yargs**: Alternative CLI framework - choose one, not both
- **inquirer**: Older prompt library - @clack/prompts is modern replacement
- **chalk**: Color library - picocolors is lighter and faster

**File System Libraries:**

- **fs-extra**: Promise-based fs with ensureDir, copy, remove
- **fast-glob**: Glob pattern matching with filters
...
- ❌ Missing `p.isCancel()` checks after prompts - causes undefined behavior on Ctrl+C, user cancellation silently becomes undefined values
- ❌ Using magic numbers for exit codes - makes debugging impossible, scripts cannot reliably check exit status
- ❌ Not handling SIGINT - leaves processes in unknown state, orphaned spinners, corrupted state files
- ❌ Synchronous file operations blocking the event loop - freezes UI, spinners stop animating

**Medium Priority Issues:**

- ⚠️ Missing spinner feedback for operations > 500ms - users assume CLI is frozen
- ⚠️ Not using `parseAsync()` for async actions - swallows errors, unhandled promise rejections
- ⚠️ Inconsistent error message formatting - confuses users, harder to parse programmatically
- ⚠️ Missing `--help` descriptions for options - poor discoverability, users forced to read source

**Common Mistakes:**

- Forgetting to call `process.exit()` after `p.cancel()` - execution continues unexpectedly
- Not validating early - errors occur deep in flow after partial writes
- Mixing sync and async patterns - leads to race conditions and deadlocks
- Not cleaning up on errors (spinners left running) - terminal output garbled

**Gotchas & Edge Cases:**

- Commander converts `--my-option` to `myOption` in camelCase - use `optsWithGlobals()` carefully
- `optsWithGlobals()` needed to access parent command options - regular `opts()` misses global flags
- Spinner must be stopped before any console output - otherwise output is overwritten
- YAML parsing can throw - always wrap in try/catch with user-friendly error messages
- `process.exit()` in async context may not wait for cleanup - use `finally` blocks for critical cleanup
