...
<p className="text-ellipsis overflow-hidden">Truncated text with ellipsis</p>
```
...
```tsx
<div className="@container">
  <div className="@2xs:text-sm @xs:text-base">Custom container breakpoints</div>
</div>
```
...
<div className="perspective-distant group">
  <div className="relative transform-3d transition-transform duration-500 group-hover:rotate-y-180">
    {/* Front face */}
    <div className="absolute inset-0 backface-hidden">Front of card</div>
    {/* Back face */}
...
<div className="bg-[--brand-color]">{/* This syntax changed in v4 */}</div>
```
...
```tsx
// v3 - ! at the beginning
<div className="!flex !bg-red-500">{/* This syntax changed in v4 */}</div>
```
