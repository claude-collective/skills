...
```tsx
<div className="p-[8px] rounded-[4px] text-[rgba(0,0,0,0.8)]">{children}</div>
```
...
export const LightPromoBanner = ({
  ...
  return (
    <div
      className={clsx("relative flex items-center gap-4 p-4", className)}
      {...rest}
    >
      {/* content */}
    </div>
...
```tsx
<LightPromoBanner title="Welcome" className="bg-blue-50 border-blue-200" />
```
...
export const Alert = ({ children }: AlertProps) => {
  return (
    <div className="fixed-styles-that-cannot-be-overridden">{children}</div>
  );
};
```
