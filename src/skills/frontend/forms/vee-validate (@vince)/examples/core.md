...
const { value: name } = useField<string>('name', (value) => {
  ...
  if (value.length < MIN_NAME_LENGTH) return `At least ${MIN_NAME_LENGTH} characters`;
...
<template>
  <form @submit="onSubmit">
    ...
    <div>
      <label for="email">Email</label>
      <input id="email" v-model="email" type="email" :aria-invalid="!!errors.email" />
      <span v-if="errors.email" role="alert">{{ errors.email }}</span>
    </div>
...
const { handleSubmit, meta, isSubmitting, resetForm, defineField, values } = useForm({
  validationSchema: schema,
...
<template>
  <form @submit="onSubmit">
    ...
    <div class="form-actions">
      ...
      <button type="submit" :disabled="!meta.valid || !meta.dirty || isSubmitting">
        {{ isSubmitting ? 'Saving...' : 'Save' }}
      </button>
    </div>

    <p v-if="meta.dirty" class="unsaved-warning">
