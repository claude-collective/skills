...
<template>
  <form @submit="onSubmit">
    ...
    <div>
      ...
      <span v-if="errors.confirmPassword" role="alert">{{ errors.confirmPassword }}</span>
    </div>
...
const schema = toTypedSchema(
  v.object({
    ...
    age: v.pipe(
      v.number(),
      v.minValue(MIN_AGE, `Must be ${MIN_AGE}+`)
    ),
  })
);
...
const schema = computed(() =>
  toTypedSchema(
    z.object({
      ...
      email: values.contactMethod === 'email'
        ? z.string().email('Invalid email')
        : z.string().optional(),
      phone: values.contactMethod === 'phone'
